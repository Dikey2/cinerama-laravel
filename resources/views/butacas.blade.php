        ...
        <aside class="bg-white md:w-1/3 border-r border-gray-300 p-6">
            <div class="flex items-center gap-4 mb-4">
                ...
                <div>
                    ...
                    <p class="text-sm text-gray-700"@extends('layouts.app')
    ...
    <div class="max-w-6xl mx-auto bg-white rounded-xl shadow-lg flex flex-col md:flex-row overflow-hidden">
        ...
        <aside class="bg-white md:w-1/3 border-r border-gray-300 p-6">
            ...
            <div class="mt-6 text-center">
                <p id="contador-reserva" class="text-red-600 font-bold mb-2"></p>
                <div class="w-full bg-gray-200 rounded-full h-2.5">
                    <div id="barra-progreso" class="bg-red-500 h-2.5 rounded-full transition-all duration-1000" style="width: 0%;"></div>
                </div>
            </div>
        </aside>
...
function butacasApp() {
    return {
        ...
        ocupadas: @json($butacasOcupadas ?? []), // ✅ vienen desde Laravel
        ...
        continuar() {
            ...
            .then(data => {
                if (data.success) {
                    ...
                    const tiempoLimite = new Date(data.limite).getTime();
                    const contador = document.getElementById("contador-reserva");
                    ...
                    const interval = setInterval(() => {
                        ...
                        const porcentaje = Math.max(0, (restante / tiempoTotal) * 100);
                        barra.style.width = `${porcentaje}%`;

                        if (restante <= 0) {
                            clearInterval(interval);
                            contador.textContent = "⏰ Tiempo expirado, liberando butacas...";
                            barra.style.width = "0%";
                            alert("⏰ Tu reserva ha expirado. Las butacas fueron liberadas.");

                            fetch("{{ route('reservas.liberar') }}", {
                                method: "POST",
                                headers: {
                                    "X-CSRF-TOKEN": "{{ csrf_token() }}",
                                    "Content-Type": "application/json",
        ...
        continuar() {
            ...
            .then(data => {
                if (data.success) {
                    ...
                    const contador = document.getElementById("contador-reserva");
                    const barra = document.getElementById("barra-progreso");
                    const tiempoTotal = 5 * 60 * 1000; // 5 minutos en milisegundos
